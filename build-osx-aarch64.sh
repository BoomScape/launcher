...
APPBASE="build/macos-aarch64/BoomScapeTestServer.app"
...
build() {
    ...
    cp native/build-aarch64/src/BoomScapeTestServer $APPBASE/Contents/MacOS/
    cp target/BoomScapeTestServer.jar $APPBASE/Contents/Resources/
    ...
    echo Setting world execute permissions on BoomScapeTestServer
    pushd $APPBASE
    chmod g+x,o+x Contents/MacOS/BoomScapeTestServer
    popd

    otool -l $APPBASE/Contents/MacOS/BoomScapeTestServer
}
...
dmg() {
    ...
    mv BoomScapeTestServer\ *.dmg BoomScapeTestServer-aarch64.dmg

    # dump for CI
    hdiutil imageinfo BoomScapeTestServer-aarch64.dmg

    if ! hdiutil imageinfo BoomScapeTestServer-aarch64.dmg | grep -q "Format: ULFO" ; then
        echo Format of dmg is not ULFO
        exit 1
    fi

    if ! hdiutil imageinfo BoomScapeTestServer-aarch64.dmg | grep -q "Apple_HFS" ; then
        echo Filesystem of dmg is not Apple_HFS
        exit 1
    fi

    # Notarize app
    if xcrun notarytool submit BoomScapeTestServer-aarch64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
        xcrun stapler staple BoomScapeTestServer-aarch64.dmg
    fi
}
