...
APPBASE="build/macos-aarch64/BoomPSTestServer.app"

build() {
    echo Launcher sha256sum
    shasum -a 256 build/libs/BoomPSTestServer.jar
    ...
    cp native/build-aarch64/src/BoomPSTestServer $APPBASE/Contents/MacOS/
    cp build/libs/BoomPSTestServer.jar $APPBASE/Contents/Resources/
    ...
    echo Setting world execute permissions on BoomPSTestServer
    pushd $APPBASE
    chmod g+x,o+x Contents/MacOS/BoomPSTestServer
    popd

    otool -l $APPBASE/Contents/MacOS/BoomPSTestServer
}
...
dmg() {
    ...
    mv BoomPSTestServer\ *.dmg BoomPSTestServer-aarch64.dmg

    # dump for CI
    hdiutil imageinfo BoomPSTestServer-aarch64.dmg

    if ! hdiutil imageinfo BoomPSTestServer-aarch64.dmg | grep -q "Format: ULFO" ; then
        echo Format of dmg is not ULFO
        exit 1
    fi

    if ! hdiutil imageinfo BoomPSTestServer-aarch64.dmg | grep -q "Apple_HFS" ; then
        echo Filesystem of dmg is not Apple_HFS
        exit 1
    fi

    # Notarize app
    if xcrun notarytool submit BoomPSTestServer-aarch64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
        xcrun stapler staple BoomPSTestServer-aarch64.dmg
    fi
}
