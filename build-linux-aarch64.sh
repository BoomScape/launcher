...
chmod 644 target/BoomScapeTestServer.jar

cp native/build-aarch64/src/BoomScapeTestServer build/linux-aarch64/
cp target/BoomScapeTestServer.jar build/linux-aarch64/
...
# Symlink AppRun -> BoomScapeTestServer
ln -s BoomScapeTestServer AppRun

# Ensure BoomScapeTestServer is executable to all users
chmod 755 BoomScapeTestServer
popd
...
ARCH=arm_aarch64 ./appimagetool-x86_64.AppImage \
	...
	build/linux-aarch64/ \
	BoomScapeTestServer-aarch64.AppImage
