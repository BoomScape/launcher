...
// kernel32
static HMODULE WINAPI BoomScapeTestServerK32LoadLibraryA(LPCSTR lpLibFileName) {
...
static HMODULE WINAPI BoomScapeTestServerK32LoadLibraryExA(LPCSTR lpLibFileName, HANDLE hFile, DWORD dwFlags) {
    if (isBlacklistedDll(lpLibFileName)) {
        SetLastError(ERROR_NOT_SUPPORTED);
        return nullptr;
    }
...
static HMODULE WINAPI BoomScapeTestServerK32LoadLibraryW(LPCWSTR lpLibFileName) {
    if (isBlacklistedDll(lpLibFileName)) {
        SetLastError(ERROR_NOT_SUPPORTED);
        return nullptr;
    }
...
static HMODULE WINAPI BoomScapeTestServerK32LoadLibraryExW(LPCWSTR lpLibFileName, HANDLE hFile, DWORD dwFlags) {
    if (isBlacklistedDll(lpLibFileName)) {
        SetLastError(ERROR_NOT_SUPPORTED);
        return nullptr;
    }
...
// kernelbase
static HMODULE WINAPI BoomScapeTestServerKBLoadLibraryA(LPCSTR lpLibFileName) {
    if (isBlacklistedDll(lpLibFileName)) {
        SetLastError(ERROR_NOT_SUPPORTED);
        return nullptr;
    }
...
static HMODULE WINAPI BoomScapeTestServerKBLoadLibraryExA(LPCSTR lpLibFileName, HANDLE hFile, DWORD dwFlags) {
    if (isBlacklistedDll(lpLibFileName)) {
        SetLastError(ERROR_NOT_SUPPORTED);
        return nullptr;
    }
...
static HMODULE WINAPI BoomScapeTestServerKBLoadLibraryW(LPCWSTR lpLibFileName) {
    if (isBlacklistedDll(lpLibFileName)) {
        SetLastError(ERROR_NOT_SUPPORTED);
        return nullptr;
    }
...
static HMODULE WINAPI BoomScapeTestServerKBLoadLibraryExW(LPCWSTR lpLibFileName, HANDLE hFile, DWORD dwFlags) {
    if (isBlacklistedDll(lpLibFileName)) {
        SetLastError(ERROR_NOT_SUPPORTED);
        return nullptr;
    }
...
#define LoadAndAttach(module, name)                                     \
    do {                                                                \
        ...
        if (module##name != nullptr) {                                  \
            DetourAttach(&module##name, BoomScapeTestServer##module##name);        \
        }                                                               \
    } while (0)
...
#define Detach(module, name)                                     \
    do {                                                         \
        if (module##name != nullptr) {                           \
            DetourDetach(&module##name, BoomScapeTestServer##module##name); \
        }                                                        \
    } while (0)
